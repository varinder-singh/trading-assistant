...
const saveProfile = async () => {
  try {
    ...
    } else if ((kiteApiKey.value && !kiteApiSecret.value && !hasSavedApiSecret.value) || (!kiteApiKey.value && kiteApiSecret.value)) {
      ...
      <section class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div class="p-6 space-y-6">
          ...
          <form v-else @submit.prevent="saveProfile" class="space-y-6">
            ...
            <div v-if="profileSuccess" class="p-3 bg-green-50 text-green-700 text-sm rounded-lg border border-green-100 flex items-center gap-2">
            ...
            <div class="space-y-4">
              ...
              <p class="text-xs text-gray-500">
                ...
                <a href="https://developers.kite.trade/" target="_blank" class="text-indigo-600 hover:underline">Get credentials here</a>.
              </p>
      ...
      <section class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div class="p-6">
          ...
          <div v-else class="space-y-4">
            ...
            <div class="border border-gray-200 rounded-xl p-5 flex flex-col md:flex-row md:items-center justify-between gap-4 hover:border-gray-300 transition-colors">
              ...
              <div class="flex items-center gap-4">
                <template v-if="connections.find(c => c.brokerName === 'zerodha')?.isActive">
                  <div class="flex items-center gap-2 px-3 py-1 bg-green-50 text-green-700 rounded-full text-sm font-medium border border-green-100">
                    <ShieldCheck class="w-4 h-4" />
                    Connected as {{ connections.find(c => c.brokerName === 'zerodha')?.brokerUserId }}
                  </div>
                  <a href="/api/auth/kite/login" class="text-sm text-indigo-600 hover:text-indigo-700 font-medium px-3 py-1.5 hover:bg-indigo-50 rounded-lg transition-colors">
                    Reconnect
                  </a>
                </template>
                <template v-else>
                  <div class="flex items-center gap-2 px-3 py-1 bg-gray-100 text-gray-600 rounded-full text-sm font-medium border border-gray-200">
                    Not Connected
                  </div>
                  <a href="/api/auth/kite/login" class="px-4 py-2 bg-gray-900 hover:bg-gray-800 text-white text-sm font-medium rounded-lg transition-colors shadow-sm">
                    Connect Account
                  </a>
                </template>
              </div>
