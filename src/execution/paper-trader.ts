...
export class PaperTrader extends EventEmitter {
      ...
      try {
        ...
        for (const trade of openTrades) {
          ...
          if (existing) {
            ...
            const totalCost =
              Number(existing.avgEntryPrice) * existing.quantity +
        ...
        if(
          this.todayTradeCount >= this.maxDailyTrades ||
          this.todayRealizedPnL <= this.maxDailyLoss
        ) {
          console.log(
  ...
  private startPositionManager() {
    ...
    console.log(
      `[Risk Manager] Starting periodic position re-evaluation every ${intervalMins} minutes...`,
    );
      ...
      async () => {
        ...
        if (positions.length === 0) {
          eventHub.emit("agent_update", {
            ...
            data: {
              reason:
                "Waiting for the AI agents to execute a new trade. Risk limits are reset and ready.",
            },
          });
          return;
        }
        ...
        for (const pos of positions) {
          try {
            ...
            if (
              pos.optionExpiry &&
              pos.strike &&
              (pos.symbol.endsWith("CE") || pos.symbol.endsWith("PE"))
            ) {
            ...
            if (decision.decision === "EXIT") {
              ...
              let newPremiumTarget =
                pos.currentPrice + optionRiskPoints * (decision.riskRewardRatio || 2.0);
              ...
              if (agentType === "TREND") {
                newPremiumTarget =
                  pos.currentPrice +
                  optionRiskPoints *
                    (decision.riskRewardRatio ? decision.riskRewardRatio * 1.5 : 4.0);
              }
        ...
        console.table(
          positions.map((p) => ({
            ...
            PnL:
              p.unrealizedPnL >= 0 ? `+${p.unrealizedPnL.toFixed(2)}` : p.unrealizedPnL.toFixed(2),
            ...
            "Burn/Day": p.optionTheta
              ? `₹${Math.abs(p.optionTheta * p.quantity).toFixed(2)}`
              : "N/A",
          })),
        );
        console.log(
  ...
  private checkMarketStatus() {
    ...
    if (hours === 15 && minutes === 25) {
      ...
      if (positions.length > 0) {
        console.log(
          `[EXIT] 15:25 IST - Market Square-off triggered for ${positions.length} positions.`,
        );
        this.squareOffAll();
      }
    }
    ...
    try {
      if (params.side === "BUY") {
        ...
        if (this.tradingHalted) {
          console.log(
            `❌ [PAPER TRADE] Trading halted for the day (Limits reached). Skipping ${params.symbol}`,
          );
        ...
        if (strike) {
          const existingStrike = Array.from(this.positions.values()).find(
            (p) => p.strike === strike,
          );
          if (existingStrike) {
        ...
        if (numLots > 1) {
          ...
          console.log(
            `⚖️ Sizing: Buying 1 lot (${params.quantity} qty) with lot size ${baseLotSize}.`,
          );
        }
      ...
      if (params.side === "SELL") {
        if (this.exitingPositions.has(params.symbol)) {
          console.log(
            `⚠️ [PAPER TRADE] Exit already in progress for ${params.symbol}. Skipping duplicate exit.`,
          );
      ...
      if (this.tradeMode === "REAL") {
        console.log(
          `⚡ [LIVE TRADE] Placing real order for ${params.quantity}x ${params.symbol} (${params.side})`,
        );
      ...
      if (order.side === "BUY") {
        await tradeRepo
          .insertTrade({
            ...
            strategy_context: params.context?.strategyContext
              ? JSON.stringify(params.context.strategyContext)
              : null,
      ...
      console.log(
        `📝 [PAPER TRADE] ${order.side} ${order.quantity}x ${order.symbol} @ ${order.price}`,
      );
  ...
  private async updatePosition(order: TradeOrder, context?: TradeContext) {
    ...
    if (order.side === "BUY") {
      if (existing) {
        ...
        const totalCost =
          existing.avgEntryPrice * existing.quantity + order.price! * order.quantity;
        existing.avgEntryPrice = totalCost / totalQty;
        existing.quantity = totalQty;
      ...
      if (existing) {
        ...
        if (targetTrade) {
          ...
          console.warn(
            `⚠️ [DB SYNC ISSUE] Could not find OPEN trade in database for ${order.symbol} to close it.`,
          );
        }
